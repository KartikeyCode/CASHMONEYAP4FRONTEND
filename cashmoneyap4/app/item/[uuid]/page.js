...
export default async function Item({ params: { uuid } }) {
  const formdata = { item_uuid: uuid };
  const resp = await getItem(formdata);
  console.log(resp.data.item);
  const image = resp.data.item.image;
  const name = resp.data.item.name;
  const description = resp.data.item.description;
  const price = resp.data.item.price;
